...
async function createAgent(req, resp) {
  ...
  const [isAgentCreate, msz] = await Agent.createNewAgent(
    fullName,
    userName,
    password,
    address,
...
async function getAllAgent1(req, resp) {
  let allAgents = await Agent.getAllAgents();

  resp.status(201).send(allAgents);
  return;
}
...
async function updateAgent(req, resp) {
  ...
  if (
    newPayload.role != "agent" &&
    newPayload.role != "employee" &&
  ...
  if (isEmployeeEsists) {
    ...
    const { agentToUpdate, propertyToUpdate, value } = req.body;

    const [isUpdate, msz] = await Agent.updateAgent(
      agentToUpdate,
      propertyToUpdate,
      value
    );
...
async function deleteAgent(req, resp) {
  ...
  if (isEmployeeEsists) {
    ...
    Agentfind.isActive == true
      ? (Agentfind.isActive = false)
      : (Agentfind.isActive = true);
...
async function getAllAgentrefer(req, resp) {
  ...
  resp
    .status(201)
    .send([allAgentsrefer.slice(startIndex, endIndex), allAgentsrefer]);
  return;
}
